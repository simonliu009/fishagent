...
class RuntimeBoundaryTests(unittest.TestCase):
    ...
    def test_console_keeps_core_views_and_adds_operational_views(self) -> None:
        ...
        self.assertIn('<title>智渔</title>', response.text)
        self.assertNotIn('<title>智渔 Agent 2.0</title>', response.text)
        ...
        self.assertIn('data-view="alerts"', response.text)
        self.assertIn('id="view_alerts"', response.text)
        self.assertIn('id="alert_history_list"', response.text)
        self.assertIn('id="alert_history_load_more"', response.text)
        self.assertIn('function incidentCardMarkup', response.text)
        self.assertIn('function renderAlertHistory', response.text)
        self.assertIn('function loadMoreAlertHistory', response.text)
        self.assertIn('按本地日期倒序查看', response.text)
        ...
        self.assertIn('双传感器告警', response.text)
        self.assertNotIn('双传感器失效', response.text)
        self.assertNotIn("injectDemo('success')", response.text)
        ...
        self.assertIn('正在加载数据，请稍候', response.text)
        self.assertIn('id="app_loading"', response.text)
        self.assertIn('class="app-loading-dots"', response.text)
        app_loading = response.text[response.text.index('<div id="app_loading"'):response.text.index('<header class="app-header"')]
        self.assertNotIn('loader-circle', app_loading)
        self.assertIn('function finishInitialLoading', response.text)
        ...
        self.assertIn('今日告警', response.text)
        self.assertIn('今日任务', response.text)
        self.assertIn("title:'养殖概览'", response.text)
        self.assertIn("metricStat('养殖池'", response.text)
        self.assertIn('所属养殖场：', response.text)
        self.assertIn('metricRatio(todayActiveIncidents.length', response.text)
        self.assertIn('class="metric-copy${item.multiline ? \' multiline\' : \'\'}"', response.text)
        self.assertIn('在线率 ${Math.round', response.text)
        self.assertIn('class="alert-flow"', response.text)
        self.assertIn('function openMetricView', response.text)
        self.assertIn('const interactive = item.destination ?', response.text)
        self.assertIn("openMetricView('${item.destination}')", response.text)
        self.assertIn('id="work_list"', response.text)
        self.assertIn('function workEntries', response.text)
        self.assertIn('function renderWorkPage', response.text)
        self.assertIn('function loadMoreWork', response.text)
        self.assertIn('id="refresh_work_button"', response.text)
        self.assertIn('onclick="refreshWorkTasks()"', response.text)
        self.assertIn('function refreshWorkTasks', response.text)
        self.assertIn('.work-toolbar { align-items: flex-end;', response.text)
        self.assertIn('function isVerificationRun', response.text)
        self.assertIn('if (run.goal === \'执行全场巡查\' || isVerificationRun(run)) return;', response.text)
        self.assertIn('查看人工任务、设备审批和 Agent 运行任务', response.text)
        self.assertLess(response.text.index('data-view="work"'), response.text.index('data-view="assistant"'))
        self.assertLess(response.text.index('id="view_alerts"'), response.text.index('id="view_work"'))
        self.assertIn('function manualTaskMarkup', response.text)
        self.assertIn('告警异常数据', response.text)
        self.assertIn('人工执行清单', response.text)
        self.assertIn('.status-badge.manual-task-status.pending { background: var(--red); }', response.text)
        self.assertIn('.status-badge.manual-task-status.completed { background: var(--brand); }', response.text)
        self.assertIn('.button.report-result { color: #fff; background: var(--blue);', response.text)
        self.assertIn('提交处置数据', response.text)
        self.assertIn('查看数据', response.text)
        self.assertIn('photo_attachments', response.text)
        ...
        self.assertIn('function openTaskPhoto', response.text)
        self.assertIn('createTaskPhotoThumbnail', response.text)
        self.assertIn('task_photo_progress_bar', response.text)
        self.assertIn('.task-photo-lightbox { z-index: 70; padding: 0;', response.text)
        self.assertIn('.task-photo-dialog { position: relative; width: 100vw; height: 100vh;', response.text)
        self.assertIn('max-width: 100vw; max-height: 100vh;', response.text)
        self.assertIn('<section class="task-photo-dialog"', response.text)
        self.assertNotIn('class="auth-dialog task-photo-dialog"', response.text)
        self.assertIn('image.naturalWidth', response.text)
        self.assertIn("closeLlmDialog(); closeTaskPhoto();", response.text)
        self.assertIn('thumbnail_object_name', response.text)
        ...
        self.assertNotIn('id="task_report_photo_evidence"', response.text)
        self.assertNotIn('onclick="completeTask(', response.text)
        self.assertIn('id="task_report_layer"', response.text)
        self.assertIn('function openTaskReport', response.text)
        self.assertIn('/api/v1/manual-tasks/${encodeURIComponent(taskReportTaskId)}/report', response.text)
        self.assertIn('数据齐全后任务自动完成', response.text)
        self.assertIn("if (run.goal === '执行全场巡查' || isVerificationRun(run)) return;", response.text)
        ...
        self.assertIn('optimisticDismissedIncidentIds', response.text)
        self.assertIn('reconcileOptimisticDismissals', response.text)
        self.assertIn('告警已标记为手动消除，正在同步', response.text)

...
The skill never accepts free-form model text as an executable command and never bypasses idempotency, evidence, or approval checks. An unhealthy device is a visible execution risk, but it does not by itself block an already validated L1 `EXECUTE`; publish the MQTT command and preserve the health warning in the result.
